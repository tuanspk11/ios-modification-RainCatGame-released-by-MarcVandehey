...
class GameScene: SceneNode, QuitNavigation, SKPhysicsContactDelegate {
  ...
  // tạo biến foodIndex để lưu giá trị foodIndex trong FoodSprite
  // foodIndex này có chức năng đưa vị trí tương ứng của loại trái cây được random thả xuống qua SoundManager trước khi FoodSprite random giá trị mới cho foodIndex
  ...
  @objc func spawnFood() {
    ...
    if !containsFood {
      // giá trị foodIndex của GameScene sẽ được gán bằng giá trị foodIndex của FoodSprite
      // để lưu lại giá trị foodIndex của loại trái cây được tạo ngẫu nhiên thả xuống trước khi FoodSprite random giá trị mới cho foodIndex
  ...
  @objc func handleCatCollision(contact: SKPhysicsContact) {
    ...
    case FloorCategory:
      cat.isGrounded = true
    default:
      // bắt sự kiện tại thời điểm mèo ăn trái cây, gọi phương thức eatFood() trong CatSprite để thực thi phương thức soundFoodName() trong SoundManager
      cat.eatFood()
//      print("Something hit the cat")
  ...
  @objc func handleFoodHit(contact: SKPhysicsContact) {
    var otherBody : SKPhysicsBody
