...
class UmbrellaSprite : SKSpriteNode, Palettable {
  ...
  @objc var clickArea : SKAControlSprite?
  ...
  @objc public func updatePosition(point : CGPoint) {
    position = point
    destination = point
  }

  @objc public func setDestination(destination : CGPoint) {
  ...
  @objc public func update(deltaTime : TimeInterval) {
  ...
  @objc public func getVelocity() -> CGVector {
    return CGVector(dx: destination.x - position.x, dy: destination.y - position.y)
  }

  @objc public func getHeight() -> CGFloat {
